...
class HumansImageDataset(BaseDataset, metaclass=ABCMeta):
    """Human Image Dataset.
    ...
    Args:
        ...
        convention (str, optional): keypoints convention. Keypoints will be
            converted from "human_dHumanDataata" to the given one.
            Default: "human_data"
    ...
    def load_annotations(self):
        ...
        if use_human_data:
            self.human_data = MultiHumanData.fromfile(self.ann_file)
    ...
    def prepare_raw_data(self, idx: int):
        ...
        idx = self.human_data['frame_range'][idx][0]
        info = {'keypoints2d': [],
                'keypoints3d': [],
                'bbox_xywh': [],
                'smpl_body_pose': [],
        ...
        for human_idx in range(self.human_data['frame_range'][sample_idx][0],
            ...
            if 'bbox_xywh' in self.human_data:
                ...
                info['bbox_xywh'].append(np.zeros((5)))
